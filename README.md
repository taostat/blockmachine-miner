...
| `eth` (Ethereum mainnet) | `latest`, `archive-reth` | reth |
...
| Resource | Latest (reth `--full`) | Archive Reth (reth default) |
|----------|------------------------|-----------------------------|
| Architecture | x86_64 | x86_64 |
| CPU | 4+ cores | 8+ cores recommended |
| RAM | 16 GB+ | 32 GB+ recommended |
| Disk | 250 GB NVMe | ~3 TB NVMe |
| Sync time | Hours (P2P) | ~24-48h from snapshot |
| Ports | 80, 443, 30303, 9000/tcp+udp, 9001/udp | 80, 443, 30303, 9000/tcp+udp, 9001/udp |
| Serves | Recent `eth_*` + `eth_subscribe` over WS | All `eth_*` for any historical block (bodies, receipts, logs) + state proofs within the 8241-block window |
...
- **Archive Reth:** full historical bodies, receipts, logs, headers, and state. Serves all customer ETH RPC traffic. Generates `eth_getProof` against any block Helios needs to verify, configured via `--rpc.eth-proof-window=8241` (50 blocks beyond the EIP-2935 8,191-block window).

Any VPS or dedicated server with Docker will work for `tao` and ETH Latest. Archive Reth benefits substantially from NVMe storage. The install script handles all dependencies (Docker, git, certificates).
...
- Ask for tier (`lite`/`archive` for tao, `latest`/`archive-reth` for eth)
- Offer to restore from a snapshot (tao archive only — eth archive operators restore a reth snapshot manually before re-running)
...
- **eth archive-reth** — Reth metrics from `:9101`, plus a beacon-API sync probe against Lighthouse on `:5052`
...
This tier serves all customer ETH RPC traffic on the network: every `eth_*` method against any historical block. Reth runs with `--rpc.eth-proof-window=8241` (50 blocks beyond the EIP-2935 8,191-block window), so `eth_getProof` answers any depth Helios can verify on the network.
...
# docker compose -f docker-compose.eth-archive.yml up -d   # eth archive-reth (reth default + lighthouse)
...
- **chain node** — Either a Bittensor subtensor (lite/archive) or a reth execution client driven by Lighthouse over the Engine API (eth latest with `--full`, or eth archive-reth in default mode).
...
| `ETH_TIER` | (empty) | eth | `latest` or `archive-reth` |
| `BACKEND_HTTP_PORT` | `8545` | eth | EL HTTP JSON-RPC port |
| `BACKEND_WS_PORT` | `8546` | eth | EL WebSocket port |
| `CHECKPOINT_SYNC_URL` | `https://mainnet.checkpoint.sigp.io` | eth | Lighthouse beacon checkpoint source |
...
docker compose -f docker-compose.eth-archive.yml down  # eth archive-reth
```
